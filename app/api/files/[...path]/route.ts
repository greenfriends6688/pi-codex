...
import {
  ...
  renameEntry,
  validateEntryName,
  // PR #899 的 `write` action。与下面 `@/lib/markdown-file` 的 writeTextFile 同名：
  // 那个是「编辑器保存」（带 baseContent 冲突检测），这个是「直接覆盖写」。两边都要，
  // 在本文件里给它起个别名。
  writeTextFile as writeFileContents,
...
async function handleFileMutation(
  ...
  const outcome = writeFileContents(targetPath, body.content, allowedRoots);
  if (!outcome.ok) return mutationErrorResponse(outcome);
  return NextResponse.json({ ok: true });
}

...
export interface SessionData {
  ...
  treeFormat?: "summary";
  /** Server dropped a stale wrapper; SSE should reconnect to the rebuilt one. */
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleAgentEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      ...
      case "prompt_done":
        ...
        break;
      case "prompt_error": {
        const failure = (event.errorMessage as string | undefined) ?? "Command failed";
      ...
      case "message_update": {
        ...
        if (!agentRunningRef.current) break;
        // fork:upstream-0.9.2-pi087-transcript — 转写 system 消息（prompt + 工具装载）已由服务端
        // 过滤；万一漏到客户端也不能进聊天记录。
  ...
  }, [isNew, newSessionCwd, newSessionModel, session, ensureNewSession, ensureEventsConnected, promoteNewSession, waitForPromptSettlement, addNotice, cancelEventStreamGrace, closeEvents, composerDraftKey, reconcileAgentState, restoreSubmission, dispatch, onAgentError]);
    ...
    try {
      switch (commandName) {
        ...
        case "auto-compact": {
          ...
          // fork:upstream-0.9.2-auto-compact — 读 live wrapper（闲会话会因此启动），
          // 否则第一次切换会跟着 React 默认值 true 走，而忽略 settings.json 的真实值。

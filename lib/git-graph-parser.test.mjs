...
async function loadSubject() {
  return import("./git-graph-parser.ts");
}

const SEP = "\x1f";

test("parses unit-separator separated git log lines", async () => {
  const { parseGitLog } = await loadSubject();
  const raw = [
...
test("subjects containing pipe characters survive the parse", async () => {
  const { parseGitLog } = await loadSubject();
  // 原型的 "|" 分隔符在这里会拆错，%x1f 必须不受影响。
  const raw = ["abc123", "", "Alice", "1700000000", "fix: a | b || c", ""].join(SEP);
  const [commit] = parseGitLog(raw);
  assert.equal(commit.subject, "fix: a | b || c");
});

test("skips malformed lines and tolerates a missing timestamp", async () => {
  const { parseGitLog } = await loadSubject();
  const raw = [
...
test("formatGitLogCommand requests topo order and the unit-separator format", async () => {
  const { formatGitLogCommand } = await loadSubject();

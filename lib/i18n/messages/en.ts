...
"sidebar.reviewChanges": "Changes ({count} files)",
    ...
    "memory.title": "Memory",
    "memory.subtitle": "The agent's long-term memory is the npm package pi-memory: markdown files it can write, read and search across sessions.",
    ...
    "memory.packageHint": "Installed through pi's package manager, so the TUI sees the same memory. New sessions pick it up; sessions already open keep their tool list until reloaded.",
    "memory.tools": "Tools it registers",
    "memory.toolsHint": "memory_write / memory_read keep durable facts; scratchpad holds things to come back to; memory_search needs the qmd binary (semantic search is optional).",
    "memory.files": "Memory files · {count}",
    "memory.filesEmpty": "No files yet — they appear after the first session writes something.",
    "memory.filesHint": "Plain markdown under the agent directory: read it here, edit it with any editor, commit it if you want it versioned. The panel is read-only on purpose — the agent owns memory.",

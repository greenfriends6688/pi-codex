...
"sidebar.reviewChanges": "Changes ({count} files)",
    ...
    "memory.subtitle": "Long-term notes the agent keeps across sessions: what to remember, when to learn automatically, and what it has saved so far.",
    "memory.switches": "Switches",
    "memory.enable": "Enable memory",
    "memory.enableHint": "Off by default. While off, nothing is injected into prompts and the remember / recall tools refuse to run.",
    "memory.autoLearn": "Learn automatically when a run finishes",
    "memory.autoLearnHint": "Runs one extra capture turn after each normal run to save durable facts. Experimental, and it spends extra tokens.",
    "memory.statusOn": "Memory is on",
    "memory.statusOff": "Memory is off — the agent neither reads nor writes notes",
    "memory.statusAuto": "auto-learn on",
    "memory.statusManual": "manual only",
    ...
    "memory.stats": "{total} entries · {global} global · {projectCount} projects · {characters} chars",
    "memory.statsSources": "{user} from you · {agent} from the agent",
    "memory.consolidate": "Consolidate",
    "memory.consolidateHint": "Trims whitespace and drops entries that say the same thing. Local and deterministic — no model call.",
    "memory.consolidated": "Removed {count} duplicate(s)",
    "memory.clearAll": "Delete everything",
    "memory.clearConfirm": "Delete all saved memories?",
    "memory.cleared": "Removed {count} entr(ies)",
    "memory.export": "Export MEMORY.md",
    "memory.exportDone": "Written to {path}",
    "memory.preview": "What gets injected",
    "memory.title": "Memory",
    "memory.add": "Add a memory",
    "memory.placeholder": "This repo uses pnpm, not npm. Tests live next to the source.",
    "memory.scope": "Scope",
    ...
    "memory.empty": "Nothing saved yet. The agent adds notes here when you ask it to remember something.",
    "memory.edit": "Edit",
    "memory.delete": "Delete memory",
    "memory.source.user": "you",
    "memory.source.agent": "agent",
    "memory.hint": "Memories are injected into every session of that scope, and the agent can add or search them with the remember / recall tools.",

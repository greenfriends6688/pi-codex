...
"sidebar.reviewChanges": "变更（{count} 个文件）",
    ...
    "memory.packageHint": "通过 pi 的包管理器安装，所以 TUI 里看到的是同一份记忆。新会话立即生效；已经打开的会话要重新加载才会带上它的工具。",
    "memory.tools": "它注册的工具",
    "memory.toolsHint": "memory_write / memory_read 存长期事实；scratchpad 放待回看的事；memory_search 需要外部 qmd 命令（语义搜索是可选的）。",
    "memory.files": "记忆文件 · {count}",
    "memory.filesEmpty": "还没有文件——第一次写入后会出现。",
    "memory.filesHint": "就是 agent 目录下的纯 markdown：在这里读、用任何编辑器改、想版本化就提交。面板刻意只读——记忆归 agent 管。",

...
"sidebar.reviewChanges": "變更（{count} 個檔案）",
    ...
    "memory.packageHint": "透過 pi 的套件管理器安裝，所以 TUI 看到的是同一份記憶。新工作階段立即生效；已開啟的工作階段要重新載入才會帶上它的工具。",
    "memory.tools": "它註冊的工具",
    "memory.toolsHint": "memory_write / memory_read 存長期事實；scratchpad 放待回看的事；memory_search 需要外部 qmd 指令（語意搜尋是選用的）。",
    "memory.files": "記憶檔案 · {count}",
    "memory.filesEmpty": "還沒有檔案——第一次寫入後會出現。",
    "memory.filesHint": "就是 agent 目錄下的純 markdown：在這裡讀、用任何編輯器改、想版本化就提交。面板刻意唯讀——記憶歸 agent 管。",

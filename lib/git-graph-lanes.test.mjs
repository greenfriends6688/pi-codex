...
async function loadSubject() {
  return import("./git-graph-lanes.ts");
}
...
// 原型示例历史的拓扑（新提交在前）：主线带两个 merge，其中一个是
// 两条实验分支的 octopus merge。
const DEMO_HISTORY = [
  commit("f541", ["f39b"]),
...
test("linear history stays on lane 0 with straight edges", async () => {
  const { buildGitGraphLayout } = await loadSubject();
...
test("the prototype history lays out merges, branches and the root correctly", async () => {
  const { buildGitGraphLayout } = await loadSubject();
  ...
  // octopus merge 的额外 parent 打开 lane 1、2。
...
test("faithful never recycles lanes while compact does", async () => {
  const { buildGitGraphLayout } = await loadSubject();
...
test("edges to parents outside the truncated window are dropped", async () => {
  const { buildGitGraphLayout } = await loadSubject();
...
test("an empty history yields an empty layout", async () => {
  const { buildGitGraphLayout } = await loadSubject();

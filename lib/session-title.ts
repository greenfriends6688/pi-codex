 ...
 * 不传时沿用会话自身的模型，行为与上游默认一致。
 */
// fork:session-title-bounded
...
export async function generateSessionTitle(
  ...
  const request = buildTitleRequest(sourceAgent, transcript);
  if (modelSelection) {
    // 影子流的 streamFunction / getApiKey 与 provider 无关，换掉 model 即可
    // 让命名走指定模型，有界转录算法本身不动。
    const selected = source.modelRuntime?.getModel(modelSelection.provider, modelSelection.modelId);
    if (!selected) {

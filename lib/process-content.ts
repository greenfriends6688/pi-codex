...
function displayableAssistantBlocks(message: AssistantMessage, isStreaming: boolean): AssistantContentBlock[] {
  // Deferred thinking only holds a preview; the full text is loaded lazily by
  // MessageView's ThinkingBlock, which the grouped path has no session id for.
  // Emitting it would render an empty/no-op reasoning row, so drop it here.
  return (message.content ?? []).filter((block) => (
    block.type !== "thinking" || isStreaming || block.thinking.trim().length > 0
  ));
}

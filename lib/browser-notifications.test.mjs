...
/*
 * fork:desktop-shell — when the Electron bridge is present the native path wins: it is
 * the only delivery that shows up under the app's own name while the window is hidden,
 * and the click is handled by the main process (it focuses the window and sends the url
 * back), so the renderer's own onClick must NOT fire on that path.
 */
test("prefers the native bridge and skips the web deliveries", async () => {
  const { showBrowserNotification } = await loadSubject();
  const sent = [];
  let clicked = false;
  const delivery = await showBrowserNotification(
    { title: "t", body: "b", sessionUrl: "/?session=abc", onClick: () => { clicked = true; } },
    {
      nativeNotify: async (payload) => { sent.push(payload); return true; },
      createWindowNotification: () => { throw new Error("web notification must not be used"); },
      getServiceWorkerRegistration: async () => ({ showNotification: async () => { throw new Error("sw must not be used"); } }),
    },
  );

  assert.equal(delivery, "native");
  assert.deepEqual(sent, [{ title: "t", body: "b", url: "/?session=abc" }]);
  assert.equal(clicked, false, "the main process owns the click");
});

test("falls back to the web delivery when the bridge refuses", async () => {
  const { showBrowserNotification } = await loadSubject();
  const shown = [];
  const delivery = await showBrowserNotification(
    { title: "t", body: "b", sessionUrl: "/", onClick: () => {} },
    {
      nativeNotify: async () => false,
      createWindowNotification: (title) => { shown.push(title); return { onclick: null, close: () => {} }; },
      getServiceWorkerRegistration: null,
    },
  );
  assert.equal(delivery, "window");

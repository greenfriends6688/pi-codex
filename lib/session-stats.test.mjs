...
test("includes cache-warming usage entries and counts transcript system messages like the SDK", () => {
  const entries = [
    {
      type: "message",
      id: "sys",
      parentId: null,
      timestamp: "2026-01-01T00:00:00.000Z",
      message: { role: "system", content: "", sections: { preamble: "prompt" }, toolsAdded: [], timestamp: 1 },
    },
    userEntry("u1", "sys"),
    {
      type: "usage",
      ...
      provider: "test",
      model: "test-model",
      usage: usage({ input: 0, output: 1, cacheRead: 100, cacheWrite: 0, cost: { input: 0, output: 0, cacheRead: 0.01, cacheWrite: 0, total: 0.01 } }),
    },
    assistantEntry("a1", "warm", [{ type: "text", text: "ok" }], usage()),
    { type: "context_edit", id: "edit", parentId: "a1", timestamp: "2026-01-01T00:00:02.000Z", targetId: "a1", replacement: null },
  ];

  const stats = computeSessionStats(entries);

  assert.equal(stats.tokens.input, 10);
  assert.equal(stats.tokens.output, 1 + 5);
  assert.equal(stats.tokens.cacheRead, 100 + 2);
  assert.equal(stats.tokens.cacheWrite, 1);
  assert.ok(Math.abs(stats.cost - (0.01 + 0.18)) < 1e-9);
  assert.equal(stats.userMessages, 1);
  assert.equal(stats.assistantMessages, 1);
  // The SDK counts every message entry, including the system message.

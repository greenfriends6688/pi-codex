...
test("Windows terminals launch PowerShell, preferring PowerShell 7 when it is on PATH", async () => {
  const require = createRequire(import.meta.url);
  const paths = await jiti.import("./paths.ts");
  const source = readFileSync(new URL("./terminal-manager.ts", import.meta.url), "utf8");
  const { outputText } = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } });

  const spawnOn = (env, pwshInstalled) => {
    const spawnCalls = [];
    const fakePty = { onData() {}, onExit() {}, write() {}, resize() {}, kill() {} };
    const fakeRequire = (id) => {
      if (id === "node-pty") return { spawn: (...args) => { spawnCalls.push(args); return fakePty; } };
      if (id === "./paths") return paths;
      if (id === "fs" || id === "node:fs") return { ...require("fs"), existsSync: () => pwshInstalled };
      return require(id);
    };
    const exports = {};
    runInNewContext(outputText, {
      exports,
      process: { ...process, env, once() {}, platform: "win32" },
      require: fakeRequire,
      setTimeout,
      clearTimeout,
      console,
    });
    const id = exports.createTerminal("C:\\workspace", 80, 24);
    exports.killTerminal(id);
    return spawnCalls[0];
  };

  // Windows PowerShell 5.1 ships with Windows 10+ and is always the fallback;
  // the old %ComSpec% (cmd.exe) behaviour is gone even when ComSpec is set.
  const bundled = spawnOn({ Path: "C:\\Windows\\System32", ComSpec: "C:\\Windows\\System32\\cmd.exe" }, false);
  assert.equal(bundled[0], "powershell.exe");
  // 沙箱里的数组来自另一个 realm，deepEqual 会比原型，逐项断言
  assert.equal(bundled[1].length, 1);
  assert.equal(bundled[1][0], "-NoLogo");

  const modern = spawnOn({ Path: "C:\\Windows;C:\\Program Files\\PowerShell\\7" }, true);
  assert.equal(modern[0], "pwsh.exe");

  const overridden = spawnOn({ Path: "C:\\Windows", PI_WEB_WINDOWS_SHELL: "C:\\tools\\nu.exe" }, false);
  assert.equal(overridden[0], "C:\\tools\\nu.exe");
});

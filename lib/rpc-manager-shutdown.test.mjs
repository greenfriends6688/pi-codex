...
test("get_state falls back to the session-level prompt for sessions with no transcript system message", async (t) => {
  // fork:upstream-0.9.2-pi087-transcript — Pi 0.86+ replays `agent.state.systemPrompt` from the
  // transcript's system messages, so a session created before that upgrade reads back empty.
  // The SDK's own AgentSession getter still knows the prompt that would be sent.
  const inner = makePromptInner(() => Promise.resolve());
  Object.defineProperty(inner.agent.state, "systemPrompt", {
    get: () => "",
    enumerable: true,
  });
  inner.systemPrompt = "Pi built prompt";
  const wrapper = new AgentSessionWrapper(inner);
  t.after(() => wrapper.destroy());

  const state = await wrapper.send({ type: "get_state" });
  assert.equal(state.systemPrompt, "Pi built prompt");
});

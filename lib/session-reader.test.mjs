// fork:external-session-write — upstream-port marker
...
test("reuses unchanged session managers and invalidates after external appends", (t) => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-session-manager-cache-"));
  ...
  writeFileSync(filePath, `${JSON.stringify(header)}\n${JSON.stringify(userEntry("u1", null, "first"))}\n`);
  ...
  const first = openSessionManager(filePath);
  const second = openSessionManager(filePath);
  assert.strictEqual(second, first);
  assert.equal(first.getEntries().length, 1);

  appendFileSync(filePath, `${JSON.stringify(assistantEntry("a1", "u1", "reply"))}\n`);
  const third = openSessionManager(filePath);
  assert.notStrictEqual(third, first);
  assert.equal(third.getEntries().length, 2);
});

// Parsed entries retain roughly the session file's own size in heap (an 82MB
// session measured ~94MB), so a count-only cap would let a dozen large
// sessions pin over a gigabyte. These cover the byte budget that prevents it.

function sessionDir(t, prefix) {
  const dir = mkdtempSync(join(tmpdir(), prefix));
  globalThis.__piSmCache = undefined;
  const limits = { ...SM_CACHE_LIMITS };
  t.after(() => {
    Object.assign(SM_CACHE_LIMITS, limits);
    invalidateSessionManagerCache();
    rmSync(dir, { recursive: true, force: true });
  });
  return dir;
...
/** A session file padded to at least `bytes` with an ignorable trailing field. */
function writeSessionOfSize(dir, name, bytes) {
  const filePath = join(dir, name);
  const header = { type: "session", version: 3, id: name, timestamp: "2026-01-01T00:00:00.000Z", cwd: dir };
  const entry = { ...userEntry("u1", null, "first"), pad: "" };
  let text = `${JSON.stringify(header)}\n${JSON.stringify(entry)}\n`;
  if (text.length < bytes) {
    entry.pad = "x".repeat(bytes - text.length);
...
test("a session larger than the per-file cap is served fresh instead of cached", (t) => {
  const dir = sessionDir(t, "pi-web-sm-cache-oversize-");
  SM_CACHE_LIMITS.maxFileBytes = 4096;

  const small = writeSessionOfSize(dir, "small.jsonl", 0);
  assert.strictEqual(openSessionManager(small), openSessionManager(small), "small session should be cached");

  const huge = writeSessionOfSize(dir, "huge.jsonl", 8192);
  const first = openSessionManager(huge);
  const second = openSessionManager(huge);
  assert.notStrictEqual(second, first, "oversize session must not be cached");
  // ...and it must not have evicted the small one that was already held.
  assert.strictEqual(openSessionManager(small), openSessionManager(small));
  assert.equal(globalThis.__piSmCache.size, 1);
});

test("the byte budget evicts least-recently-used sessions", (t) => {
  const dir = sessionDir(t, "pi-web-sm-cache-budget-");
  SM_CACHE_LIMITS.maxFileBytes = 8192;
  SM_CACHE_LIMITS.maxTotalBytes = 5000;   // holds two 2KB sessions, not three

  const a = writeSessionOfSize(dir, "a.jsonl", 2000);
  const b = writeSessionOfSize(dir, "b.jsonl", 2000);
  const cachedA = openSessionManager(a);
  const cachedB = openSessionManager(b);
  assert.equal(globalThis.__piSmCache.size, 2);

  // Touch `a` so `b` becomes the least-recently-used entry.
  assert.strictEqual(openSessionManager(a), cachedA);

  const c = writeSessionOfSize(dir, "c.jsonl", 2000);
  openSessionManager(c);
  assert.equal(globalThis.__piSmCache.size, 2, "adding a third session must evict one");
  assert.strictEqual(openSessionManager(a), cachedA, "recently used session survives");
  assert.notStrictEqual(openSessionManager(b), cachedB, "the least-recently-used session was evicted");
});

test("the entry-count cap still applies to small sessions", (t) => {
  const dir = sessionDir(t, "pi-web-sm-cache-count-");
  SM_CACHE_LIMITS.maxEntries = 3;

  const held = [];
  for (let i = 0; i < 5; i++) held.push(writeSessionOfSize(dir, `s${i}.jsonl`, 0));
  for (const filePath of held) openSessionManager(filePath);
  assert.equal(globalThis.__piSmCache.size, 3);
});
...
test("uses the latest explicit model change instead of assistant response metadata", () => {
  const entries = [
    {
      type: "model_change", id: "model", parentId: null,
      provider: "gateway", modelId: "claude-sonnet-5", timestamp: "2026-01-01T00:00:00.000Z",
    },
    userEntry("user", "model", "hello"),
    {
      ...assistantEntry("assistant", "user", "hello"),
      message: {
        role: "assistant", provider: "gateway", model: "anthropic/claude-sonnet-5",
        content: [{ type: "text", text: "hello" }],
      },
    },
  ];

  assert.deepEqual(buildSessionContext(entries).model, {
    provider: "gateway",
...
test("allowStale serves the previous scan and refreshes in the background", async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  let backgroundScanFinished = false;
  let releaseBackgroundScan;
  const backgroundScanGate = new Promise((resolve) => {
    releaseBackgroundScan = resolve;
  });
  listSessions = async () => {
    scans += 1;
    if (scans === 2) {
  ...
  // Ordinary agent activity invalidates the catalogue without the caller doing
  // anything, so a search must not pay for a rebuild every time.
  invalidateSessionListCache();
  await listAllSessions({ allowStale: true });
  assert.equal(scans, 2, "a stale read must schedule the rebuild");
  assert.equal(backgroundScanFinished, false, "a stale read must not wait for the rebuild");

  releaseBackgroundScan();
  for (let i = 0; i < 40 && !backgroundScanFinished; i += 1) await new Promise((resolve) => setTimeout(resolve, 5));
  assert.equal(backgroundScanFinished, true, "the background rebuild must still finish");
});

test("allowStale still rebuilds when there is no previous scan to serve", async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  listSessions = async () => {
    scans += 1;
  ...
  // Nothing cached: the first read has to scan, stale or not.
  await listAllSessions({ allowStale: true });
  assert.equal(scans, 1);
});

test("an invalidated catalogue still rebuilds for callers that need fresh data", async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  listSessions = async () => {
    scans += 1;
  ...
  invalidateSessionListCache();
  await listAllSessions();
  assert.equal(scans, 2, "the sidebar must not be served an invalidated catalogue");
});

test("allowStale reuses a fresh catalogue without scanning", async (t) => {
  const originalListSessions = listSessions;
  let scans = 0;
  listSessions = async () => {
    scans += 1;
...
test("a scan invalidated in flight retries before returning to its caller", async (t) => {  const originalListSessions = listSessions;
  let scans = 0;
...
test("hides transcript system messages and metadata entries while keeping entry ids aligned", () => {
  ...
  // System messages never consume the visible tail budget either.
  const tail = buildSessionContext(entries, undefined, { tail: 2 });
  assert.deepEqual(tail.entryIds, ["a1", "u2"]);
});

...
// fork:upstream-0.9.2-turn-anchor — #891 移植的单测
...
test("getAssistantTruncationNotice returns a notice for stopReason length", async () => {
  const { getAssistantTruncationNotice } = await loadSubject();

  const message = {
    ...assistant([{ type: "thinking", thinking: " lengthy reasoning " }]),
    stopReason: "length",
  };
  const notice = getAssistantTruncationNotice(message);
  assert.ok(notice, "expected a truncation notice for stopReason=length");
  assert.match(notice, /output limit/i);
});

test("getAssistantTruncationNotice returns null while streaming", async () => {
  const { getAssistantTruncationNotice } = await loadSubject();

  const message = {
    ...assistant([{ type: "thinking", thinking: "still going" }]),
    stopReason: "length",
  };
  assert.equal(getAssistantTruncationNotice(message, { isStreaming: true }), null);
});

test("getAssistantTruncationNotice returns null for non-length stopReason", async () => {
  const { getAssistantTruncationNotice } = await loadSubject();

  assert.equal(
    getAssistantTruncationNotice({ ...assistant([]), stopReason: "stop" }),
    null,
  );
  assert.equal(
    getAssistantTruncationNotice({ ...assistant([]), stopReason: "error", errorMessage: "oops" }),
    null,
  );
  assert.equal(
    getAssistantTruncationNotice({ ...assistant([]) }),
    null,
  );
});

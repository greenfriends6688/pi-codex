...
async function loadSubject() {
  try {
    ...
    return createJiti(import.meta.url).import("./provider-icon.ts");
  } catch {
    return import("./provider-icon.ts");
  }
}
...
test("parseProviderIconMode accepts only api/letter/emoji and defaults to auto", () => {
  assert.equal(parseProviderIconMode("api"), "api");
  assert.equal(parseProviderIconMode("letter"), "letter");
  assert.equal(parseProviderIconMode("emoji"), "emoji");
...
test("resolveProviderIconSource emoji mode uses the stored emoji", () => {
  assert.deepEqual(resolveProviderIconSource("deepseek", null, "emoji", true, "🚀"), {
    type: "emoji",
    emoji: "🚀",
  });
  // emoji 模式优先于 logo/API，和 letter 模式一样。
  assert.deepEqual(resolveProviderIconSource("my-proxy", "openai-completions", "emoji", false, "⚡"), {
    type: "emoji",
    emoji: "⚡",
  });
});

test("resolveProviderIconSource emoji mode degrades to letter without data", () => {
  assert.deepEqual(resolveProviderIconSource("deepseek", null, "emoji", true, null), {
    type: "letter",
    letter: "D",
  });
  assert.deepEqual(resolveProviderIconSource("deepseek", null, "emoji", true, undefined), {
    type: "letter",
    letter: "D",
  });
  assert.deepEqual(resolveProviderIconSource("deepseek", null, "emoji", true, "   "), {
...
test("resolveProviderIconSource ignores the emoji argument in other modes", () => {
  assert.deepEqual(resolveProviderIconSource("deepseek", null, "auto", true, "🚀"), {
    type: "provider-logo",
  });
  assert.deepEqual(resolveProviderIconSource("my-proxy", "openai-completions", "auto", false, "🚀"), {
...
test("firstGrapheme keeps emoji sequences whole", () => {
  ...
  if ("Segmenter" in Intl) {
    // ZWJ 序列和旗帜必须保持为一个 grapheme cluster。
    assert.equal(firstGrapheme("👨‍💻🚀"), "👨‍💻");
    assert.equal(firstGrapheme("🇨🇳🇺🇸"), "🇨🇳");
  }
});

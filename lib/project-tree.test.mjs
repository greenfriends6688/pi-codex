...
test("does not preview transcript system messages", () => {
  // Pi >= 0.86 stores the prompt as a system message at the root and before tool changes.
  const system = (id) => ({ type: "message", id, parentId: null, timestamp: "t", message: { role: "system", content: "", sections: { preamble: "You are an expert coding assistant." }, toolsAdded: [] } });
  const arm = node(system("s1"), [node(msg("u1", "user", "第一个分支"), [node(info("leaf1"))])]);
  const sibling = node(system("s2"), [node(msg("u2", "user", "另一个分支"), [node(info("leaf2"))])]);
  const projected = projectTreeForResponse([node(info("root"), [arm, sibling])]);

  assert.deepEqual(findProjectedNode(projected, "leaf1").branchPreview, { role: "user", text: "第一个分支" });
  assert.deepEqual(findProjectedNode(projected, "leaf2").branchPreview, { role: "user", text: "另一个分支" });
  assert.equal(JSON.stringify(projected).includes("expert coding assistant"), false);
});

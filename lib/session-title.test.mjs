// fork:session-title-bounded — upstream-port marker
...
test("routes the title request to an explicitly selected model", async () => {
  let seenModel;
  const selected = { provider: "other", id: "chosen-model", reasoning: false };
  const sourceSession = {
    agent: {
      state: {
        systemPrompt: "system",
        ...
        thinkingLevel: "off",
        tools: [],
        messages: [{ role: "user", content: "Name me", timestamp: 1 }],
      },
      waitForIdle: async () => {},
      convertToLlm: (messages) => messages,
      streamFunction: (model) => {
        ...
        const stream = createAssistantMessageEventStream();
        queueMicrotask(() => {
          stream.push({ type: "done", reason: "stop", message: assistantMessage("Chosen Model Title") });
        });
        return stream;
      },
      sessionId: "source-session-id",
    ...
    modelRuntime: {
      getModel: (provider, modelId) => (
        provider === "other" && modelId === "chosen-model" ? selected : undefined
      ),
    },
  };

  const result = await generateSessionTitle(sourceSession, { provider: "other", modelId: "chosen-model" });
  assert.equal(result.title, "Chosen Model Title");
  assert.equal(seenModel, selected);
});

test("rejects a model selection the registry does not know", async () => {
  await assert.rejects(
    () => generateSessionTitle({
      agent: {
        state: {

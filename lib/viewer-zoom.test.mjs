...
test("withPdfZoom keeps an existing PDF page fragment", () => {
  const base = "/api/files/x.pdf?type=read";
  assert.equal(withPdfZoom(`${base}#page=7`, 1.5), `${base}#page=7&zoom=150`);
  assert.equal(withPdfZoom(`${base}#page=7&zoom=150`, 2), `${base}#page=7&zoom=200`);
});

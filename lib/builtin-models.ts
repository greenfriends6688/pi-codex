 ...
 * provider-composer 对 `models[]` 是「整条替换」而非合并，用户写一个同名模型会
 * 丢掉内置的 `thinkingLevelMap` / `compat`，因此保存时给出「覆盖内置定义」警告。
 *
 * 前端接线（app/api/models-config/route.ts PUT）：保存成功的响应里会多一个可选字段
 * `warnings: string[]`（元素形如 `provider/modelId`），非空时应在保存成功提示中展示。
 * 旧客户端忽略该字段即可，保持向后兼容。
 */
...
/** 判断 models.json 的 models[] 条目是否覆盖同名内置模型（返回 `provider/modelId` 冲突列表）。 */

...
test("认领超过 10 分钟没有心跳才视为过期", () => {
  const now = new Date("2026-09-21T09:00:00.000Z");
  assert.equal(CRON_CLAIM_STALE_MS, 10 * 60 * 1000);
  assert.equal(claimIsStale({ lastStatus: "running", heartbeatAt: "2026-09-21T08:51:00.000Z" }, now), false);
  assert.equal(claimIsStale({ lastStatus: "running", heartbeatAt: "2026-09-21T08:49:59.000Z" }, now), true);
  // No heartbeat yet: fall back to the run start.
  assert.equal(claimIsStale({ lastStatus: "running", lastRunAt: "2026-09-21T08:40:00.000Z" }, now), true);
  assert.equal(claimIsStale({ lastStatus: "running", lastRunAt: "2026-09-21T08:55:00.000Z" }, now), false);
  // Only a task that claims to be running can be stale, and corrupt data is never reclaimed.
  assert.equal(claimIsStale({ lastStatus: "ok", lastRunAt: "2026-09-01T00:00:00.000Z" }, now), false);
  assert.equal(claimIsStale({ lastStatus: "running" }, now), false);
  assert.equal(claimIsStale({ lastStatus: "running", lastRunAt: "not-a-date" }, now), false);
});

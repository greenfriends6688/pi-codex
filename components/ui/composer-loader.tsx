...
// fork:ds-chat-motion（DS-24）—— composer 忙碌光环：三段虚线沿 pill 描边绕行 + feGaussianBlur 泛光。
// 来源：BoardUI components/application/composer-loader/composer-loader.tsx（MIT）。改动：默认渐变色从上游的
// hex 常量改为 BoardUI 语义 token（--color-border-focus-ring / --color-notification-*-foreground），两色中点
// 混合因此兼容非 hex；ResizeObserver / pathLength / feGaussianBlur / 450ms 淡入淡出逐行保留；无 motion、
// 无 remixicon、无新增依赖（上游本文件亦未用到前两者）。
...
/**
 * 默认渐变色。上游是固定 hex 虹彩；本仓换成 BoardUI 语义 token（跟随明暗）。
 * 这仍是“组件数据默认值”而非样式类：调用方可用 `colors` 覆盖成任意 CSS 颜色（含 hex）。
 */
const DEFAULT_COLORS: [string, string, string, string] = [
  "var(--color-border-focus-ring)",
  "var(--color-notification-information-foreground)",
  "var(--color-notification-error-foreground)",
  "var(--color-notification-success-foreground)",
];
...
/**
 * 上游把两个中间色做 RGB 中点，避免光带中心发白。默认色现在是 token 变量，
 * 所以非 hex 输入退化为 `color-mix(in oklab, …)`；hex 输入保持上游算法。
 */
function midpoint(a: string, b: string): string {
  if (HEX_TRIPLET.test(a) && HEX_TRIPLET.test(b)) {
    const ca = hexTriplet(a);
    const cb = hexTriplet(b);
    return `rgb(${ca.map((value, index) => Math.round((value + cb[index]) / 2)).join(",")})`;
  }
  return `color-mix(in oklab, ${a} 50%, ${b})`;
}
  ...
  return (
    <div className={cx("relative", className)}>
      {surface && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-0 bg-background-primary-default shadow-xs"

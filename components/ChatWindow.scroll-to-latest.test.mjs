...
test("fades the button in and out without motion when motion is reduced", () => {
  ...
  // fork:boardui PR-21 — 过渡时长收敛到 --motion-* token，这里两种写法都接受。
  assert.match(cssSource, /@media \(prefers-reduced-motion: reduce\) \{\s*\.chat-scroll-to-bottom,\s*\.chat-scroll-to-bottom\.is-visible \{\s*transform: none;\s*transition: background (?:0\.12s|var\(--motion-fast\)), color (?:0\.12s|var\(--motion-fast\));/);
});

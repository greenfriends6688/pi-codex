...
### PROMA-23 · 会话恢复降级（context replay）

> 本条目是第二轮核对 Proma 核心清单时补的。它为单独一个能力，但与 P2 其他条目无依赖，且实现最简单。

- **目标**：续接 pi 会话失败时不再直接报错，而是降级为“重放上下文”——注入最近 20 条消息 + 会话元信息 + 完整历史文件路径，并要求 agent 自己 Read 完整历史后继续。覆盖三类真实故障：session 文件丢失、上下文超长、thinking-signature 不匹配。

- **依据（Proma）**：
  - `buildContextPrompt`（无 resume 的会话）：最近 20 条 + 会话元信息 + 历史文件路径 + 指示读完整历史：`agent-session-context-prompt.ts:5, 94-150`
  - `buildRecoveryPrompt`（resume 失败时）：`:152-181`
  - 恢复处理与重试：`agent-orchestrator.ts:1815-1930`

- **接入点**：
  - `lib/rpc-manager.ts`：`startRpcSession` 失败时（`SessionManager.open` 抛错 / 文件不存在）捕获并转入降级路径
  - 新 `lib/context-replay.ts`（构造重放提示词，纯函数 + `.test.mjs`）
  - 新 `lib/session-recovery.ts`（错误分类：session-not-found / prompt-too-long / thinking-signature；决定是否降级）
  - `lib/session-reader.ts`：复用现有分页读取能力拼“最近 20 条”
  - UI：降级发生时给一条可见提示，**不要静默**

- **注意**：
  - **降级必须可见**。静默重放会让用户以为 agent“失忆”是 bug；Proma 有 banner，我也要有。
  - 重放只做一次（防死循环）；二次失败正常报错。
  - 只把**历史文件路径**给 agent 自己去读，**不要**把全量历史塞进提示词（会立即爆上下文）。
  - thinking-signature 错误在切换 provider/模型后最常见，值得优先覆盖。
  - `lib/prompt-recovery.ts` 是另一回事（失败后恢复上一条提示词的去重），**不要**混用或改写它。

- **工作量**：S–M（~300 行 + 测试 + i18n ~6 key）
- **风险**：低-中。只影响失败路径：降级不触发时行为与现状完全一致。
...
- **目标**：① 编辑器内查找栏（大小写/全词/正则 + 命中计数 + Enter 导航）② 目录 + scroll-spy ③ 表格真网格编辑（单元格导航 + 右键增删行列 + 写回 markdown）。另可附带：图片预览的缩放/平移（Proma 是 10–500% + 滚轮 + 拖拽，我现在只是一个 `<dialog>` lightbox）。
...
#### PROMA-20a · 小项：回答里的外部链接路由到内嵌浏览器（可独立先做）

本项**不依赖 CDP**，也不依赖 spike 结论，与现有 iframe 面板配合即可，所以列在这里供单独抽取。

- **目标**：agent 回答里的 http(s) 链接不再一律开系统浏览器，而是进内嵌浏览器面板（按会话排队 + 活动点）；用户可以长按/中键选择“开系统浏览器”。

- **依据（Proma）**：`AgentBrowserLinkProvider.tsx:18-60`（按会话排队，路由进内嵌浏览器）；`MainArea.tsx:95-135`

- **接入点**：`components/MarkdownBody.tsx:80-85`——现在本地文件链接已经路由给 `onOpenFile`（`:80-81`），只有 http(s) 分支是 `target="_blank"`（`:85`）；把该分支改成可选回调 `onOpenUrl`，由 `ChatWindow`/`AppShell` 接到已有的 `components/browser-tab-state.ts` + `BrowserPanel`

- **注意**：
  - **必须保留“开系统浏览器”的退路**（Proma 是直接把链接路由进内嵌，用户无法选择）。同一个链接需要两种去向时，内嵌面板要有一个“在系统浏览器打开”按钮——`BrowserPanel` 工具栏已有这个按钒，复用即可。
  - 用户如果关掉了右栏/没开浏览器 tab，**不要静默丢失**：要么自动开一个 tab，要么降级为系统浏览器。
  - 不进 CDP 路径，与 PROMA-20 主项的 spike 结论解耦。

- **工作量**：S（~60 行 + 测试 + i18n ~4 key）
- **风险**：低。纯 UI 路由。
